...
======================
THIS is the module the website should use. It does NOT train anything: it
loads the already-trained models (artifacts/*.joblib, generated once with
train_export_artifacts.py) and predicts in seconds.

No chemistry knowledge required to use it: every step is a function with a
clear input and output. The function normally called from the web backend
is the last one, `predict_falcipain2(...)`.

All user-facing text (labels, JSON keys, messages) is in ENGLISH, ready to
be shown directly on an English-language website.
...
Each function can be used on its own if the web developer only needs one
...
# Display text for the two classes. Kept here (not inside the trained
# bundle) so it can be edited freely without retraining anything.
LABEL_MEANING = {
...
def read_input(data, input_type="smiles", smiles_column="smiles", id_column=None):
    """
    Normalizes the website's input into a standard table.
    ...
    if input_type == "smiles":
        ...
        if isinstance(data, (bytes, bytearray)):
            ...
            df = pd.read_csv(data)  # file path or file-like object
...
def compute_mordred_descriptors(mols):
    ...
    Can take a few seconds per molecule for large batches; for 1-10 single
    molecules from the website it is practically instant.
    """
    calc = Calculator(mordred_descriptors, ignore_3D=True)
    df_desc = calc.pandas(mols, nproc=1, quiet=True)
    # Mordred sometimes puts an error string instead of NaN for a few
    # problematic descriptors; force everything to numeric.
...
def compute_maccs_keys(mols):
    ...
    molecule. Returns a DataFrame with columns MACCS_0 ... MACCS_166.
    """
...
def align_features(df_raw, feature_columns):
    """
    Models were trained with a FIXED list of columns
    (bundle["feature_columns"]). A new molecule may generate columns in a
    different order, or may be missing one if Mordred failed to compute
    it. This function:
      - Keeps only the columns the model expects, in the right order.
      - If a column is missing, creates it empty (NaN); the model itself
        (which already includes a fitted imputer) fills those gaps with
        the median seen during training.

    Returns a DataFrame ready to feed directly into the model.
    """
...
# =============================================================================
# STEP 5 - Applicability domain (kNN-AD)
# =============================================================================
...
def check_applicability_domain(X, bundle):
    ...
    Method: kNN-AD. The descriptors are standardized with the scaler fitted
    on the training set, then the average Euclidean distance to the
    molecule's 5 nearest neighbors IN THE TRAINING SET is computed. If that
    distance is below the threshold learned during training (mean + 2*SD of
    the same distance computed within the training set itself), the
    molecule is considered within the applicability domain.
    ...
    DataFrame with columns:
      - "mean_dist_5nn" : average distance to the 5 nearest training
                          neighbors (number)
      - "in_AD"         : True/False, whether it is within the applicability domain
      - "threshold"     : kNN-AD threshold used (same for every row, informational)
    """
    scaler = bundle["ad_scaler"]
    nn = bundle["ad_nn"]
    threshold = bundle["ad_threshold"]
    k = bundle["ad_n_neighbors"]
    feature_medians = bundle["feature_medians"]

    X_num = X.apply(pd.to_numeric, errors="coerce")
    X_num = X_num.fillna(feature_medians)  # a missing descriptor is filled with the training median
    X_scaled = scaler.transform(X_num)

    # nn was fit with n_neighbors+1 (see train_export_artifacts.py); for a
    # brand-new molecule (not part of the training set) all returned
    # neighbors are genuine distinct compounds, so only the first k are kept
    # to stay consistent with how the training-set threshold was computed.
    distances, _ = nn.kneighbors(X_scaled)
    mean_dist = distances[:, :k].mean(axis=1)

    return pd.DataFrame({
        "mean_dist_5nn": mean_dist,
        "in_AD": (mean_dist < threshold),
        "threshold": threshold,
    })
...
def load_artifact_bundle(descriptor_type, artifacts_dir=ARTIFACTS_DIR_DEFAULT):
    """
    Loads the .joblib bundle generated by train_export_artifacts.py.
...
def predict_with_top3(X, bundle, active_threshold=0.5):
    ...
    Returns a DataFrame (one row per molecule) with:
      ...
      - "prediction"       : "Active" or "Inactive" (final decision, based
        on mean probability >= active_threshold)
      - "consensus"        : "unanimous" if all 3 models agree,
                              "majority" if only 2 of 3 agree
    """
    top3_models = bundle["top3_models"]  # {model_name: pipeline}
    prob_cols = {}
...
def explain_prediction(X_query_row, bundle, model_name=None, top_n=10):
    ...
    Parameters
    ----------
    X_query_row : single-row DataFrame (same structure as X from
        align_features), the molecule to explain.
    ...
    """
    import shap  # lazy import: only needed when an explanation is requested
    ...
    # Generic explainer: works with any model type (trees, linear, SVM,
    # neural nets...) because it explains the predict_proba function
    # directly, without depending on the internal architecture.
...
# =============================================================================
# ORCHESTRATOR - the only function the web developer normally needs to call
# =============================================================================
...
def predict_falcipain2(data, input_type="smiles", descriptor_types=("mordred", "maccs"),
    ...
    Parameters
    ----------
    data : whatever comes from the web form (see read_input)
    input_type : "smiles" | "csv"
    ...
    explain : if True, adds the SHAP interpretability for each molecule
        (slower; for a CSV with many molecules it may be better to set it
        to False and only offer the explanation on demand, one molecule
        at a time).
        ...
        for i in range(len(df_valid)):
            ...
            for dt in descriptor_types:
                ...
                result_dt = {
                    ...
                    "applicability_domain_distance": round(float(ad_row["mean_dist_5nn"]), 4),
